...
document.getElementById('tag-search').addEventListener('input', event => {{
  ...
  matches.forEach((t, index) => {{ const btn = document.createElement('button'); btn.dataset.id = String(t.tag_id ?? ''); btn.textContent = String(t.text ?? ''); searchResults.appendChild(btn); if (index < matches.length - 1) searchResults.appendChild(document.createElement('br')); }});
}});
