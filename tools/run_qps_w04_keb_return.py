...
import yaml
CORR="QPS-FED-W04-T10-SAFE-CTRL"; REQUEST=Path("federation/qps/QPS_FED_W04_KEB_REQUEST.yaml"); QA=Path("PIPELINE/qps_w04_keb_qa_actions.yaml"); GLOSSARY=Path("PIPELINE/GLOSSARY.yaml"); SNAPSHOT=Path("federation/qps/snapshots/QPS_FED_W04_SANITIZED_PARENT_SNAPSHOT_v0.2.yaml")
...
def main()->int:
 parser=argparse.ArgumentParser(); parser.add_argument("--child-root"); parser.add_argument("--child-sha",required=True); parser.add_argument("--snapshot",default=str(SNAPSHOT)); parser.add_argument("--output",default="federation/qps/runtime/QPS_FED_W04_KEB_RUNTIME_RECEIPT.yaml"); args=parser.parse_args()
 if not re.fullmatch(r"[0-9a-f]{40}",args.child_sha): raise SystemExit("--child-sha must be a full commit SHA")
 request,qa,glossary=load(REQUEST),load(QA),load(GLOSSARY); requested=request.get("requested_KEB_operations"); supported={"glossary_alignment","semantic_drift_check","schema_and_enum_alignment","ADR_index_exchange","evidence_reference_exchange","lifecycle_and_standard_reference_exchange","maturity_telemetry_exchange","lineage_hash_receipt"}
 ...
 if args.child_root:
  root=Path(args.child_root); evidence=list(root.rglob("QPS_TABLE10_SUPPORT_SYSTEM_COMPANION_v0.1.yaml"))+list(root.rglob("QPS_SAFETY_EVIDENCE_RTM_OFFER_CROSSWALK_v0.1.yaml")); input_mode="private_child"
 ...
 inputs=[REQUEST,QA,GLOSSARY,*evidence]; corpus="\n".join(p.read_text(encoding="utf-8") for p in evidence); governed={norm(k) for k in glossary.get("glossary",{})}; domains=request.get("exchange_domains",[]); status={}; findings=[]
