...
## Wave W000 federation/runtime bootstrap

Wave W000 adds a bootstrap contract for federation-oriented handoffs without replacing the repository's existing governance substrate.

### Boundary with `semantic_substrate/`

- `semantic_substrate/` remains the canonical, validator-backed governance layer for tuple lineage, invariants, and repository-level semantic drift scoring.
- `federation/semantic_index/schema.yaml` is a descriptive W000 schema for future dual-render handoffs; it is documented for humans now and intentionally does not claim CI enforcement yet.
- `telemetry/pca/drift_monitor.py` provides a focused completion-vector variance utility for federation telemetry, while `semantic_substrate/analytics/drift_engine.py` remains the broader repository drift evaluator.

### Bootstrap artifacts

| Path | Purpose | Current wave status |
| --- | --- | --- |
| `federation/semantic_index/schema.yaml` | Dual-render tuple contract for human/machine handoffs | W000 descriptive bootstrap |
| `agents/codex/MCP_INSTRUCTION.md` | CODEX-side federation operating directives | W000 active guidance |
| `agents/abacus/FEDERATION_PROTOCOL.md` | ABACUS-side completion-vector and gate protocol | W000 active guidance |
| `telemetry/pca/drift_monitor.py` | JSON-emitting variance monitor for completion vectors | W000 executable bootstrap |

### Drift monitor inputs, process, and output

The PCA drift bootstrap now accepts complete metric vectors from JSON files, validates that all required dimensions are present, computes mean absolute variance, and emits a structured JSON payload that captures the input files, tracked dimensions, and resulting drift score.

```bash
python telemetry/pca/drift_monitor.py \
  --current-file current_metrics.json \
...
- **W002**: connect PCA-oriented completion-vector monitoring with the broader semantic substrate drift workflows.
