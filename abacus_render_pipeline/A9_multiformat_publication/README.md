...
This prevents an HTML directory, Pages upload artifact, deployment-success signal, or stale HTTP `200` response from being counted as hosted publication evidence.
...
`receipts/multiformat_execution_receipt.json` carries deterministic pre-deployment evidence: exact source commit, SSOT and tuple-ledger hashes, per-format artifact hashes, native telemetry, semantic parity, cross-format parity and replay result.
...
`A9_3_PAGES_STABILITY_REPAIR.md` defines the repair and exact-main re-close predicate. `INV-008` still requires real hosted evidence; `INV-010` additionally requires stable repeated convergence under the repository-wide Pages deployment boundary.

A10 has separately closed `DEBT-008 snapshot_receipt` using a real Chromium PNG and image-native evidence. The A9 hosted propagation regression does not erase that snapshot proof and does not justify reopening it.
...
First re-prove A9.3 on exact `main` and close `DEBT-007` again from retained hosted evidence. After that, do not add another publication abstraction by default: A10 snapshot proof already exists, so further work should address remaining open governance/renderer debts or a concrete newly observed defect.
