...
def collect_tuples(export_dir: Path) -> list[dict[str, Any]]:
    """Return all parsed tuple dicts from ``export_dir``."""
    ...
    for yml_file in sorted(export_dir.rglob("*.yml")):
        try:
            ...
            if isinstance(data, dict):
                ...
                tuples.append(data)
        except Exception:  # noqa: BLE001 — tolerate malformed files, report below
            pass
    return tuples
...
def main(argv: list[str] | None = None) -> int:
    ...
    tuples = collect_tuples(args.export_dir)

    report = render_report(tuples, phase_map)
