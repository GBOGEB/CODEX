...
Canonical location: `book_md/10_books/20_slides/`. Deck outputs should go to `book_md/10_books/20_slides/output/` (keep out of Git).
...
1. Install the Python dependencies in an isolated environment:
   ...
   ```
2. Place your Markdown source in `book_md/10_books/20_slides/src/` and reference assets in `book_md/10_books/20_slides/assets/`.
3. Run the one-liner to generate a full deck (PPTX + PDF + HTML):
  ```bash
  python -m slides.qplant_sckcen_template build --source book_md/10_books/20_slides/src/demo.md --output-dir book_md/10_books/20_slides/output
  ```
4. Open the generated `book_md/10_books/20_slides/output/demo.pptx` in PowerPoint, or share `demo.html` for a quick review.
...
python -m slides.qplant_sckcen_template build \
  --source book_md/10_books/20_slides/src/strategy.md \
  --output-dir book_md/10_books/20_slides/output \
  --partner-logo book_md/10_books/20_slides/assets/partner.svg \
...
python -m slides.qplant_sckcen_template batch \
  --manifest-dir book_md/10_books/20_slides/manifests \
  --output-dir book_md/10_books/20_slides/output \
  --format pptx pdf html
```
...
```bash
python -m slides.qplant_sckcen_template build \
  --source book_md/10_books/20_slides/src/full_report.md \
...
| HTML images broken | Asset path not copied | Place images in `book_md/10_books/20_slides/assets` or set `--asset-root <directory>` to a shared directory. |
...
- Add the following DOW stage to render decks on every merge:
  ...
  stages:
    - name: ppt
      ...
      inputs:
        source_glob: book_md/10_books/20_slides/src/*.md
        output_dir: book_md/10_books/20_slides/output
        formats: [pptx, pdf, html]
  ```
- Upload `book_md/10_books/20_slides/output/*.pptx`, `*.pdf`, and `*.html` as pipeline artefacts; do **not** commit them to Git.
- Publish the metadata report (`book_md/10_books/20_slides/output/metadata.json`) so downstream consumers can reuse hyperlinks and authorship data.
...
```bash
python -m http.server --directory book_md/10_books/20_slides/output
```
