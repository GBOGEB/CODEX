...
function Get-ManifestMap {
    ...
    foreach ($line in Get-Content -LiteralPath $manifest) {
        ...
        if ($line -notmatch '^([0-9a-fA-F]{64})\s{2}(.+)$') { throw "Malformed manifest line: $line" }
...
if ($roundtrip.result -ne 'PASS') { throw "Roundtrip receipt must be PASS; got '$($roundtrip.result)'." }
if (-not $roundtrip.all_semantic_comparisons_pass) { throw 'Roundtrip receipt does not confirm Build A/B semantic equality.' }
if ($roundtrip.private_source_semantic_sha256 -notmatch '^[a-f0-9]{64}$') { throw 'Roundtrip receipt is missing a valid private_source_semantic_sha256.' }
if ($releaseReceipt.result -ne 'PASS') { throw "Release receipt must be PASS; got '$($releaseReceipt.result)'." }
if ($releaseReceipt.release_id -ne $ReleaseId) { throw "Release receipt ID '$($releaseReceipt.release_id)' does not match requested release '$ReleaseId'." }
...
foreach ($relative in $allPaths) {
    ...
    $status = if (-not $localExists) { 'MISSING_LOCAL' }
    elseif (-not $publishedExists) { 'MISSING_PUBLISHED' }
    elseif ($null -eq $manifestLocal) { 'NOT_IN_LOCAL_MANIFEST' }
    elseif ($null -eq $manifestPublished) { 'NOT_IN_PUBLISHED_MANIFEST' }
    elseif ($localHash -ne $manifestLocal) { 'LOCAL_MANIFEST_MISMATCH' }
    elseif ($publishedHash -ne $manifestPublished) { 'PUBLISHED_MANIFEST_MISMATCH' }
    elseif ($localHash -ne $publishedHash) { 'LOCAL_ONEDRIVE_HASH_MISMATCH' }
...
if ($localManifestHash -ne $publishedManifestHash) { $failures += 'MANIFEST_FILE_HASH_MISMATCH' }
$accepted = ($failures.Count -eq 0)

$registryLines = $files | Sort-Object path | ForEach-Object { "$($_.path)|$($_.local_sha256)|$($_.onedrive_sha256)|$($_.status)" }
$artifactRegistrySha256 = Get-Sha256Text -Text ($registryLines -join "`n")
$qaPassed = @($files | Where-Object status -eq 'PARITY_VERIFIED').Count
$qaTotal = $files.Count
...
$acceptance = [ordered]@{
    ...
    private_source_semantic_sha256 = $roundtrip.private_source_semantic_sha256
    ...
    artifact_registry_sha256 = $artifactRegistrySha256
    qa_passed = $qaPassed
    qa_total = $qaTotal
...
if ([string]::IsNullOrWhiteSpace($OutputPath)) { $OutputPath = Join-Path $LocalDist 'ACCEPTANCE_RECEIPT.json' }
...
if ($accepted -and -not [string]::IsNullOrWhiteSpace($BridgePointerOutput)) {
    $safePath = $OneDriveRelativePath.Replace('\','/')
    $pointer = @(
        'schema: qps-cost-master.release-pointer.v1',
        "release_id: '$ReleaseId'",
        "schema_version: '$SsotVersion'",
        "private_source_semantic_sha256: '$($roundtrip.private_source_semantic_sha256)'",
        "qa_passed: $qaPassed",
        "qa_total: $qaTotal",
        "release_manifest_sha256: '$localManifestHash'",
        "artifact_registry_sha256: '$artifactRegistrySha256'",
        "onedrive_relative_path: '$safePath'",
        "confidentiality: 'private data and binaries are not present in this public repository'"
    )
...
if (-not $accepted) { throw ("Accepted-release parity gate remains HOLD:`n" + ($failures -join "`n")) }
