...
class PPTXTopologyReconstructionRuntime:
    NODE_ROLES = {"TITLE", "BODY_TEXT", "FLOW_OBJECT", "TABLE", "GROUP"}
    ...
    def build_nodes(self, df: pd.DataFrame) -> List[TopologyNode]:
        ...
        for _, row in df.iterrows():
            ...
            cx, cy = self.center(row["left_mm"], row["top_mm"], row["width_mm"], row["height_mm"])
            ...
            nodes.append(
                TopologyNode(
                    ...
                    extracted_text=str(row["extracted_text"])[:300],
                )
            )
    ...
    def build_edges(self, nodes: List[TopologyNode]) -> List[TopologyEdge]:
        ...
        for i, source in enumerate(nodes):
            ...
            for j, target in enumerate(nodes):
                ...
                d = self.distance(source.center_x_mm, source.center_y_mm, target.center_x_mm, target.center_y_mm)
